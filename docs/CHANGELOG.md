...
- Expanded the offline Pokédex from generation 1 (151) to the national
  ...
  state moved to blob version 2 (272 bytes, 130-byte bitmaps) and still
  migrates a v1 52-byte save so existing records are not lost. Browse
  with UP/DOWN click (±1), double-click (±10), and long-press (jump to
...
- Replaced Pokédex catch toggling with cry playback: OK click plays the
  ...
  NVS still keeps seen + last viewed. Cries come from PokeAPI latest
  OGG via `tools/gen_pokedex_cries.py`. A merged image that includes
  `cryfs` spans past `cardid` and must not be raw-flashed from `0x0`
  onto a provisioned device.
- Added an offline Pokédex page (`demo_pokedex.c`) in the "Pokédex" menu
  entry: national-dex Pokémon are embedded in the firmware — stats,
  English flavor text, and 48×48 pixel sprites (raw-deflate compressed,
  generated by `tools/gen_pokedex_static.py` from
  [PokeAPI](https://pokeapi.co/) data). Browse with UP/DOWN, play the cry
  with OK, and persist seen state plus the last viewed entry in NVS
  so records survive power loss. The UI is a green "electronic dex" screen
  whose background (#244238) and mint-green highlights are taken from the
  reference `pokedex.guoxudong.io` web dex; sprites are pre-blended onto
  that background so there are no white borders. The 240×320 layout is a
  host-tested geometry (`pokedex_layout`): a 96×96 2× sprite well on the
  right, number/name/3-letter type chips on the
  left, height/weight, a taller flavor panel, and a two-line footer. Long
  type names no longer collide with the sprite, the battery percentage
  stays light on the dark header, and the previous three-line footer that
  overflowed the screen is gone. All Wi-Fi/HTTPS fetching, JSON streaming
  (`pokedex_stream`/miniz), and the Pokédex menuconfig options were removed;
  the JSON/stream modules and their host tests are kept only as reference.
  Host tests cover state serialization golden vectors, sprite
  blending/deflate decoding, JSON extraction, the static dex table, and
  layout bounds/overlap/abbreviation contracts.
- Made the Pokédex screen dim its backlight after 60 seconds without keys and
  restore it with any button press.
- Made the actionlint bootstrap script prefer `shasum` over macOS's BSD
  `sha256sum` shim (which lacks GNU `--check`), keeping `validate.sh --static`
  runnable on macOS.
