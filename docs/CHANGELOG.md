...
- Added an offline Pokédex page (`demo_pokedex.c`) in the "Pokédex" menu
  entry: all 151 generation-1 Pokémon are embedded in the firmware — stats,
  English flavor text, and 48×48 pixel sprites (raw-deflate compressed,
  generated by `tools/gen_pokedex_static.py` from
  [PokeAPI](https://pokeapi.co/) data). Browse with UP/DOWN, mark catches
  with OK, and persist caught/seen state plus the last viewed entry in NVS
  ...
  that background so there are no white borders, and the layout shows the
  sprite thumbnail, number/name/type badges, height/weight, and the flavor
  text together on one screen. All Wi-Fi/HTTPS fetching, JSON streaming
  ...
  blending/deflate decoding, JSON extraction, and the static dex table.
