...
- 新增 `FAP_SCREENSHOT_V1` 串口屏幕截图协议（`main/screenshot.c`）：低优先级
  后台任务监听 USB 控制台上的发布工具命令，把当前屏幕按小端 RGB565 逐帧送出
  （240×320，持 LVGL 锁期间临时包装 flush 回调，拆成自上而下 16 条 20 行条带）。
  命令仅观测：不重启、不烧录、不改设置、不输出凭据。二进制流期间静音日志、
  VFS TX 行尾临时切 LF，结束后恢复。AI Passport 社区发布工具的真机截图证据
  依赖该协议。
